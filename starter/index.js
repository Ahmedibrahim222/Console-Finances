...
function analyzeFinancialRecords(data) {
  ...
  for (let i = 0; i < totalMonths; i++) {
    ...
    if (i > 0) {
      ...
      if (change > greatestIncrease.amount) {
        greatestIncrease = { date: month, amount: change };
      }
      if (change < greatestDecrease.amount) {
        greatestDecrease = { date: month, amount: change };
      }
    }
  }
  ...
  resultsContainer.innerHTML = `
    ...
    <p>Greatest Increase in Profits/Losses: ${greatestIncrease.date} ($${greatestIncrease.amount})</p>
    <p>Greatest Decrease in Profits/Losses: ${greatestDecrease.date} ($${greatestDecrease.amount})</p>
  `;

  // Append the results to the body of the document
  document.body.appendChild(resultsContainer);
}

// Call the function with the provided dataset
analyzeFinancialRecords(finances);